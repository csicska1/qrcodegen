(function (global) {
  ...
  // (Tables and helper data omitted here for brevity, same as before)
  // ... You can keep your original ECC_CODEWORDS_PER_BLOCK and ALIGNMENT_PATTERN_LOCATIONS tables ...
    ...
    for (let dy = 0; dy < 7; dy++) {
      for (let dx = 0; dx < 7; dx++) {
        ...
        m[yy][xx] =
          ...
          (dx >= 2 && dx <= 4 && dy >= 2 && dy <= 4);     // inner dark 3×3
      }
    }
  }
  ...
  function renderToCanvas(m, scale, canvas){
    var n=m.length;
    ...
    for (var y=0;y<n;y++)
      for (var x=0;x<n;x++)
        if (m[y][x]) ctx.fillRect(x*scale,y*scale,scale,scale);
  }

  global.QRCode = {
    generate: function(text, version, ecl){
      return encode(text, version||1, ecl||"M");
    },
